...
%
% General packages
\usepackage{graphics}   % For basic graphics support
\usepackage{graphicx}   % Advanced graphics support
\usepackage{epsfig}     % For postscript graphics files (deprecated but included here)
\usepackage{xcolor}     % Colors for text, tables, etc.

% Fonts and math
\usepackage{mathptmx}   % Times font for text (includes math)
...
\usepackage{pgfplots}   % For plots (integrates with TikZ)
\pgfplotsset{compat=1.17} % Set compatibility version
...
% Captions
\usepackage{capt-of}    % For creating captions outside of floating environments
...
%\item \changes{Introduction of additional decision variables to support time/trajectory invariant constraints.}
...
\begin{mini!}
    {\textbf{x}_{2:N}, \textbf{u}_{2:N-1}, \boldsymbol{\beta}, \textbf{r}}{\changes{\sum_k} \ell(\textbf{x}_k, \textbf{u}_k, \boldsymbol{\beta}, \textbf{r}), \quad \forall\,\, \textbf{x}_1 \in \mathbb{X}_1 \label{eq:problem_obj}}
    {\label{eq:problem}}{}
...
\vspace{-10pt}
\subsection{Scalability}\label{sec:results:scaling}
% \subsubsection{Overactuated Double Integrator}
\changes{The scalability of the four methods is compared empirically using an over-actuated double integrator problem. The number of control inputs is swept from $m=2:200$ for GA and MINLP and $m=2:300$ control inputs for CAPO and MIQP to increase the difference resolution between these two methods. The CAPO formulation for this problem can be written as follows:
\begin{mini}|s|
...
\vspace{-10pt}
\subsection{Benchmark Problems}\label{sec:results:benchmark}
% \begin{table}
% \centering
% \vspace{-5pt}
